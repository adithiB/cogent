...
import { orgs, users, memberships, apiKeys } from '../db/schema';
...
import {
  generateApiKey,
  hashApiKey,
} from '../db/repositories/api-keys.repository';
...
export interface SignupView extends SessionView {
  /** ADR-0002 §2c: the ingestion bearer credential, shown once, here — it
   * is stored only as a SHA-256 hash and is never retrievable again. */
  apiKey: string;
}
...
export class AuthService {
  ...
  private readonly dummyHash = hashPassword(
    'not-a-real-password-used-for-timing-safety-only',
  );
   ...
   *
   * ADR-0002 §2c extends this transaction by one insert: the org's first
   * ingestion API key is minted atomically with the org itself, so a
   * freshly signed-up org can send events immediately with no separate
   * management step.
   */
  async signup(dto: SignupDto, res: Response): Promise<SignupView> {
  ...
  async login(
    ...
    const membership = user
      ? await this.memberships.findFirstForUser(user.id)
      : undefined;
